...
def prepare_data(coin_pair_name, number_of_samples):
    ...
    data_frame = get_last_n_rows(data_frame, number_of_samples)
...
def print_close_price(data_frame, buy_points, sell_points, coin_pair_name, number_of_samples):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"plots/{coin_pair_name}/close_price_plot_{number_of_samples}.svg")
    plt.close()
...
def print_macd_indicator(data_frame, macd_values, signal_values, coin_pair_name, number_of_samples):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"plots/{coin_pair_name}/macd_plot_{number_of_samples}.svg")
    plt.close()
...
def print_portfolio(data_frame, portfolio_values, coin_pair_name, number_of_samples):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"plots/{coin_pair_name}/portfolio_plot_{number_of_samples}.svg")
    plt.close()
...
if __name__ == "__main__":
    ...
    data = prepare_data(coin_pair_name, number_of_samples)
